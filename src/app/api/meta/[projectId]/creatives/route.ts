...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ projectId: string }> }) {
  ...
  try {
    const data = await fetchAllPages(
      `https://graph.facebook.com/${META_API_VERSION}/${channel.adAccountId}/adcreatives`,
      channel.accessToken,
      { fields: "id" },
      200
    );
    ...
    return NextResponse.json({ error: e instanceof Error ? e.message : "Erro ao buscar criativos" }, { status: 500 });
  }
}

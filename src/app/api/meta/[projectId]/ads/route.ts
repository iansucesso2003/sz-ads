...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ projectId: string }> }) {
  ...
  try {
    const data = await fetchAllPages(
      `https://graph.facebook.com/${META_API_VERSION}/${channel.adAccountId}/ads`,
      channel.accessToken,
      { fields: "id,name,status,effective_status,creative{id},created_time", effective_status: '["ACTIVE"]' }
    );
